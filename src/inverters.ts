...
export async function InvertersData() {
  ...
  try {
    console.log("Navigating to the Solarshop.co.ke/solar-inverters/...");
    await page.goto("https://solarshop.co.ke/solar-inverters/", {
    ...
    const productUrls = await page.evaluate(() => {
      ...
      productElements.forEach((product, index) => {
        try {
          ...
          console.log(
            `Error processing product ${index}:`,
    ...
    const urlsToScrape = productUrls.slice(0, 499);

    for (let i = 0; i < urlsToScrape.length; i++) {
      const productInfo = urlsToScrape[i];
      console.log(
        `Scraping product ${i + 1}/${urlsToScrape.length}: ${productInfo.name}`
      );
      ...
      try {
        ...
        console.log(
          `Error scraping ${productInfo.name}:`,
          error instanceof Error ? error.message : String(error)
        );
        detailedProducts.push({
          name: productInfo.name,
    ...
    const jsonData = {
      ...
      baseUrl: "https://solarshop.co.ke/solar-inverters/",
      scrapedAt: new Date().toISOString(),
      totalProducts: detailedProducts.length,
      products: detailedProducts,
    };

    fs.writeFileSync("Inverters_data.json", JSON.stringify(jsonData, null, 2));
    console.log("Saved to Inveters_data.json");
